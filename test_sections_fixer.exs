defmodule TestSectionsFixer do
  @moduledoc """
  Test script for demonstrating the LiveView section fixer with a realistic example.
  This script will analyze test/fixtures/missing_sections_live.ex and fix missing sections.

  Usage:
    mix run test_sections_fixer.exs preview  # Show preview of changes
    mix run test_sections_fixer.exs fix      # Fix the file
  """

  alias ExCodeAudit.Fixers.LiveView

  @file_path "test/fixtures/missing_sections_live.ex"
  @fixed_file_path "test/fixtures/missing_sections_live_fixed.ex"
  ...
  defp preview_fixes(content) do
    ...
    case LiveView.fix_sections(content, @sections_to_add, preview: true, file_path: @file_path) do
      {:ok, preview} ->
        ...
        IO.puts(IO.ANSI.cyan() <> "mix run test_sections_fixer.exs fix" <> IO.ANSI.reset())
...
# Run the script
TestSectionsFixer.run(System.argv())
