defmodule UserLiveTest do
  @moduledoc """
  Test script for demonstrating the LiveView section fixer with a realistic user_live.ex example.

  Usage:
    mix run user_live_test.exs preview  # Show preview of changes
    mix run user_live_test.exs fix      # Fix the file
  """

  alias ExCodeAudit.Fixers.LiveView

  @file_path "test/fixtures/user_live.ex"
  @fixed_file_path "test/fixtures/user_live_fixed.ex"
  @sections_to_add ["LIFECYCLE CALLBACKS", "EVENT HANDLERS", "RENDERING"]

  def run(args \\ []) do
    IO.puts(IO.ANSI.green() <> "LiveView Section Fixer - Realistic Demo" <> IO.ANSI.reset())
  ...
  defp preview_fixes(content) do
    ...
    case LiveView.fix_sections(content, @sections_to_add, preview: true, file_path: @file_path) do
      {:ok, preview} ->
        ...
        IO.puts(IO.ANSI.cyan() <> "mix run user_live_test.exs fix" <> IO.ANSI.reset())
